...
ucsc.rmsk.salmon_quant <- load.tximeta.object.list('ucsc.rmsk.salmon', output_data.dir = output_data.dir)

ucsc.rmsk.salmon_quant <- parse.tximeta.quant.metadata(ucsc.rmsk.salmon_quant)
```

# analysis_INTERACTIVE
## making the individual groups
```{r}
import::from(.directory = r_code.dir, 
             .from = 'helper_analysisFunctions.R', 
             subset.tximeta.se)

ucsc.rmsk.salmon_quant$day_5_bulk_ctrl_ucsc.rmsk <- 
  subset.tximeta.se(ucsc.rmsk.salmon_quant$day_5_bulk_ucsc.rmsk, 
                    filter_list = sample_meta_data$`2022_05_25_sample_meta_data.csv` %>% 
                       filter(condition %in% c('D5_scram', 'ctrl')) %>% pull('sample'))

ucsc.rmsk.salmon_quant$day_5_bulk_kraskd_ucsc.rmsk <- 
  subset.tximeta.se(ucsc.rmsk.salmon_quant$day_5_bulk_ucsc.rmsk, 
                    filter_list = sample_meta_data$`2022_05_25_sample_meta_data.csv` %>% 
                       filter(condition %in% c('D5_kras', 'kras')) %>% pull('sample'))

ucsc.rmsk.salmon_quant$day_5_exo_ctrl_ucsc.rmsk <- 
  subset.tximeta.se(ucsc.rmsk.salmon_quant$day_5_exo_ucsc.rmsk, 
                    filter_list = sample_meta_data$`2022_05_25_sample_meta_data.csv` %>% 
                       filter(condition == 'ctrl') %>% pull('sample'))

ucsc.rmsk.salmon_quant$day_5_exo_kraskd_ucsc.rmsk <- 
  subset.tximeta.se(ucsc.rmsk.salmon_quant$day_5_exo_ucsc.rmsk, 
                    filter_list = sample_meta_data$`2022_05_25_sample_meta_data.csv` %>% 
                       filter(grepl('kraskd', sample)) %>% pull('sample'))

ucsc.rmsk.salmon_quant$day_7_bulk_ctrl_ucsc.rmsk <- 
  subset.tximeta.se(ucsc.rmsk.salmon_quant$day_7_bulk_ucsc.rmsk, 
                    filter_list = sample_meta_data$`2022_05_25_sample_meta_data.csv` %>% 
                       filter(condition %in% c('day7', 'ctrl')) %>% pull('sample'))

ucsc.rmsk.salmon_quant$day_7_bulk_kraskd_ucsc.rmsk <- 
  subset.tximeta.se(ucsc.rmsk.salmon_quant$day_7_bulk_ucsc.rmsk, 
                    filter_list = sample_meta_data$`2022_05_25_sample_meta_data.csv` %>% 
                       filter(condition %in% c('day7', 'kras')) %>% pull('sample'))
```
...
ucsc.rmsk.salmon_quant$analysis_set$d7 <- 
  build.analysis.set(se_list = ucsc.rmsk.salmon_quant, 
                     se_1 = 'day_7_bulk_ctrl_ucsc.rmsk', 
                     se_2 = 'day_7_bulk_kraskd_ucsc.rmsk',
                     sample_meta_in = sample_meta_data$`2022_05_25_sample_meta_data.csv` %>% 
                       filter(grepl('day7', sample)))

ucsc.rmsk.salmon_quant$analysis_set$d5 <- 
  build.analysis.set(se_list = ucsc.rmsk.salmon_quant, 
                     se_1 = 'day_5_bulk_ctrl_ucsc.rmsk', 
                     se_2 = 'day_5_bulk_kraskd_ucsc.rmsk',
                     sample_meta_in = sample_meta_data$`2022_05_25_sample_meta_data.csv` %>% 
                       filter(grepl('D5', sample)))


ucsc.rmsk.salmon_quant$analysis_set$combined <- 
    build.analysis.set(se_list = ucsc.rmsk.salmon_quant, 
                     se_1 = 'day_7_bulk_ucsc.rmsk', 
                     se_2 = 'day_5_bulk_ucsc.rmsk',
                     sample_meta_in = sample_meta_data$`2022_05_25_sample_meta_data.csv` %>% 
                       filter(grepl('D5|day7', sample)))

ucsc.rmsk.salmon_quant$analysis_set$combined$quant_meta %<>% 
  separate(condition, '_', into = c(NA, 'condition')) %>% 
  mutate(condition = ifelse(condition == 'scram', 'ctrl', condition))
```
...
ucsc.rmsk.salmon_quant$analysis_set$d7$deseq <- 
  run.de.seq.individual(input_se = ucsc.rmsk.salmon_quant$analysis_set$d7,
                        type = 'gxi',
                        base_level = 'day7_kras',
                        top_level = 'day7_ctrl',
                        ref_type = 'ucsc.rmsk',
                        dds_formula = as.formula('~condition'))

ucsc.rmsk.salmon_quant$analysis_set$d5$deseq <- 
  run.de.seq.individual(input_se = ucsc.rmsk.salmon_quant$analysis_set$d5,
                        type = 'gxi',
                        base_level = 'D5_kras',
                        top_level = 'D5_ctrl',
                        ref_type = 'ucsc.rmsk',
...
ucsc.rmsk.salmon_quant$analysis_set$combined$deseq <- 
  run.de.seq.individual(input_se = ucsc.rmsk.salmon_quant$analysis_set$combined,
                        type = 'gxi',
                        base_level = 'kras',
                        top_level = 'ctrl',
                        ref_type = 'ucsc.rmsk',
...
ucsc.rmsk.salmon_quant$analysis_set$d5_kras$deseq <- 
  run.de.seq.individual(input_se = ucsc.rmsk.salmon_quant$analysis_set$d5_kras,
                        type = 'gxi',
                        base_level = 'kras',
                        top_level = 'ctrl',
                        ref_type = 'ucsc.rmsk',
                        dds_formula = as.formula('~context.y'))

ucsc.rmsk.salmon_quant$analysis_set$d5_ctrl$deseq <- 
  run.de.seq.individual(input_se = ucsc.rmsk.salmon_quant$analysis_set$d5_ctrl,
                        type = 'gxi',
                        base_level = 'ctrl',
                        top_level = 'kras',
                        ref_type = 'ucsc.rmsk',
...
# ucsc.rmsk.salmon_quant$day_5_exo_ucsc.rmsk$quant_meta$condition <-
#   c('ctrl','ctrl','ctrl','kras','kras','kras')
# ucsc.rmsk.salmon_quant$day_5_exo_ucsc.rmsk$deseq <- 
#   run.de.seq.individual(input_se = ucsc.rmsk.salmon_quant$day_5_exo_ucsc.rmsk,
#                         type = 'gxi',
#                         base_level = 'ctrl',
#                         top_level = 'kras',
#                         ref_type = 'ucsc.rmsk',
...
ltr_promoter_bed <- read_tsv(file.path(output_data.dir, 'LTR7_Dependent_Gene_List.bed'),
                             col_names = c('chr','start','stop','strand','gene'))


ltr_promoter_bed %>% 
  separate(gene, '_', into = c(NA, 'gene')) %>% 
  filter(!grepl('alt', chr)) %>% 
  select(chr, start, stop, gene, strand) %>% 
  filter(gene %in% filter(ucsc.rmsk.salmon_quant$analysis_set$d5$deseq$de_out, padj < 0.05)$gene) -> d5_ltr_genes
  write_tsv(col_names = F, file = file.path(output_data.dir, 'filt_LTR7_Dependent_Gene_List.bed'))
```
...
repeats_path <- file.path(reference.dir, 'ucsc.rmsk.salmon.gtf')

repeats_path <- system.file(package = 'Repguide', 
                            'extdata', 
                            'hg38_ucsc_rmsk_ltr.txt.gz')

gs <- createGuideSet(genome = Hsapiens, 
                     refdir = reference.dir,
                     tes = repeats_path,
                     temp = tempdir())

ltr_promoter_bed %>% 
  separate(gene, '_', into = c(NA, NA, 'gene')) %>% 
  filter(!grepl('alt', chr)) %>% 
  select(chr, start, stop, gene, strand) %>% 
  filter(gene %in% filter(ucsc.rmsk.salmon_quant$analysis_set$d7$deseq$de_out, padj < 0.05)$gene) %>% 
  mutate(repname = paste0('LTR7')) %>% 
  select(chr, start, end = stop, strand, repname) %>% 
  GenomicRanges::GRanges() -> ltr_promoter_granges

read_tsv(file.path(output_data.dir, 'targets_for_repguide.bed'), 
         col_names = c('chrom', 'start', 'end', 'gene', 'strand', 'te_id')) %>% 
  mutate(repname = 'LTR7') %>% 
  select(chrom, start, end, strand, repname, te_id) %>% 
  GenomicRanges::GRanges() -> target_coords

gs <- addTargets(gs, targets = target_coords)

# Create data.frame with repname and corresponding file path
msa_df <- data.frame('repname' = c('LTR7'), 
                     'path' = file.path(output_data.dir, 
                                        'ltr7_filt_promoter.msa'))
...
flow_chart.plt <- magick::image_read(path = file.path(figure.dir, 'fig.1', 'rer_biorender_8.08x8.3.png'))
...
salmon_pca_corr_top_7.df %>% 
  ...
  theme(legend.position = 'top', legend.justification = 'center', axis.text.x = element_text(angle = 60, hjust = 1)) + 
...
salmon_quant$analysis_set$ctrl_panc_covid$quant_meta %>% 
  ...
  mutate(num = str_split_fixed(string = sample, pattern = '[.]', n = 4)[, 2],
         sample = paste0(condition, ' ', num)) %>% 
  select(-num) %>% 
...
wrap_plots(fig.S1B.plt,
           patchwork::wrap_elements(full = fig.S1C.meta_table.grob, ignore_tag = F),
...
rowRanges(ucsc.rmsk.salmon_quant$analysis_set$d7$gxi) %>% 
  as.data.frame() %>% 
  filter(seqnames == 'chrM') %>% 
  select(group_name) %>% 
  unlist() %>% unname() -> chrM.filter.list

ucsc.rmsk.salmon_quant$analysis_set$d7$deseq$de_out %>% 
  ...
  filter(!ensg %in% chrM.filter.list,
         !grepl('^MT', gene)) -> kras_annotated_rmsk_de

kras_label_list <- kras_annotated_rmsk_de %>% 
  filter(grepl('LTR|Alu', gene)) %>% 
  top_n(6, wt = log2FoldChange)

kras_label_list_dn <- kras_annotated_rmsk_de %>% 
  filter(grepl('LTR|Alu', gene)) %>% 
  top_n(3, wt = -log2FoldChange)

kras_label_lnc_list_dn <- kras_annotated_rmsk_de %>% 
  filter(clade == 'gencode_lncRNA',
         !grepl('^ENSG', gene)) %>% 
  top_n(3, wt = -log2FoldChange)

kras_label_list <- rbind(kras_label_list, kras_label_list_dn, kras_label_lnc_list_dn)

kras_annotated_rmsk_de %>%
...
gencode_layer.plt + 
  geom_point(data = filter(kras_annotated_rmsk_de, clade != 'gencode_coding') %>% 
  ...
  ggrepel::geom_text_repel(data = kras_annotated_rmsk_de %>%
               mutate(clade = factor(clade, levels = te_aware_annotation_levels)),
                      aes(label = ifelse(abs(log2FoldChange) > 1.2 & padj < 0.05, gene, '')),
                           show.legend = F, color = 'black',
               max.overlaps = 10, box.padding = unit(4.5, 'mm'), segment.colour = 'grey')

fig.2A.plt


ucsc.rmsk.salmon_quant$analysis_set$d7$deseq$de_out %>% 
  merge(ucsc.rmsk.salmon_quant$analysis_set$d5$deseq$de_out, by = 'gene') %>% 
  merge(kras_annotated_rmsk_de %>% select(gene, clade), by = 'gene') %>% 
  filter(gene %in% d5_ltr_genes$gene) %>% 
  distinct() %>% View()
  ...
  geom_point() +
  scale_color_manual(values = te_color_pal, name = 'biotype/clade') + 
  ggrepel::geom_text_repel(aes(label = ifelse(abs(log2FoldChange.x) > 1.5, gene, '')), show.legend = F) +
  xlab('EB DE') + ylab('iPSC DE')
...
## Fig_2B
```{r}
ucsc.rmsk.salmon_quant$analysis_set$d5_ctrl$deseq$de_out %>% 
# ucsc.rmsk.salmon_quant$analysis_set$ctrl_panc_covid$deseq$covid_vs_ctrl %>% 
  ...
  mutate(family = ifelse(ensg %in% reference_meta_data$gencode_lncRNA_gene_names.df$ensg,
         ...
         clade = factor(clade, levels = te_aware_annotation_levels)) %>% 
  filter(!ensg %in% chrM.filter.list,
         !grepl('^MT', gene)) -> ctrl_annotated_rmsk_de

ctrl_label_list <- ctrl_annotated_rmsk_de %>% 
  top_n(3, wt = log2FoldChange)

ctrl_label_lnc_list_dn <- ctrl_annotated_rmsk_de %>% 
  filter(clade == 'gencode_lncRNA',
         !grepl('^ENSG', gene)) %>% 
  top_n(3, wt = -log2FoldChange)

ctrl_label_list <- rbind(ctrl_label_list, ctrl_label_lnc_list_dn)

ctrl_annotated_rmsk_de %>%
  filter(clade == 'gencode_coding') %>% 
  ggplot(aes(log2FoldChange, -log10(padj), color = clade)) + 
  geom_point(size = 2, alpha = 0.6, show.legend = FALSE) -> gencode_layer.plt

gencode_layer.plt + 
  geom_point(data = filter(ctrl_annotated_rmsk_de, clade != 'gencode_coding') %>% 
               mutate(clade = factor(clade, levels = te_aware_annotation_levels)),
             aes(log2FoldChange, -log10(padj), color = clade), size = 2, alpha = 0.8) +
  scale_color_manual(values = te_color_pal, name = 'biotype/clade') -> volcano_ctrl.fig
  # ggrepel::geom_text_repel(data = ctrl_annotated_rmsk_de %>% 
  #            mutate(clade = factor(clade, levels = te_aware_annotation_levels)),
  #                   aes(label = ifelse(gene %in% ctrl_label_list$gene, gene, '')), 
...
## Fig_2C
```{r}
ucsc.rmsk.salmon_quant$analysis_set$d5_ctrl$deseq$norm_counts.df %>% 
...
total_annotated_rmsk_counts %>% 
  ...
  merge(ucsc.rmsk.salmon_quant$analysis_set$d5_ctrl$deseq$scaled_quant_meta_for_de.df %>%
          select(sample, context.y), by = 'sample') -> tidy_total_annotated_rmsk_counts

tidy_total_annotated_rmsk_counts %>% 
  group_by(sample, context.y, clade) %>% 
  summarize(clade_sum = sum(count)) %>% 
  mutate(clade_frac = clade_sum/sum(clade_sum),
         context.y = factor(context.y, levels = c('intra', 'exo'))) %>% 
  ...
  ggforce::facet_row(~context.y, scales = 'free_x') +
  theme(axis.text.x = element_blank()) +
  xlab('sample') + ylab('normalized count fraction') -> glass_ctrl.fig
...
tidy_total_annotated_rmsk_counts %>% 
  ...
  mutate(clade_frac = clade_sum/sum(clade_sum),
         context.y = factor(context.y, levels = c('intra', 'exo'))) %>% 
  ggplot(aes(reorder(clade, clade_frac), clade_frac, fill = context.y)) + 
  geom_line(aes(group = sample), alpha = 0.25) + #, position = position_dodge(width = 0.25)) +
  geom_point(shape = 21, size = 4) + #, position = position_dodge(width = 0.25)) + 
  coord_flip() + 
...
wrap_plots(glass_ctrl.fig,
           volcano_ctrl.fig, 
           guide_area(),
           design = fig_2_layout, 
           guides = 'collect',
           heights = c(1,0.0937)) + plot_annotation(tag_levels = 'A') & 
  theme(plot.tag.position = c(0.01, 0.99), 
        legend.position = 'bottom',
...
## Fig_2D
```{r}
lineage_markers <- read_csv(file.path(output_data.dir, 'lineage_marker_genes.csv')) %>% 
  gather(lineage, marker) %>% 
  filter(marker != 'EMPTY') %>% 
  merge(reference_meta_data$total_tx_to_gene.df %>% select(ensg, gene) %>% distinct(), by.x = 'marker', by.y = 'gene') %>% 
  distinct()

lineage_markers <- lineage_markers[-53, ] # remove extra LEFTY1


ucsc.rmsk.salmon_quant$analysis_set$combined$deseq$norm_counts.df %>% 
  rownames_to_column('ensg') %>% 
  merge(lineage_markers, by = 'ensg') %>% 
  distinct() %>% 
  select(-ensg, -lineage) %>% 
  mutate_if(is.numeric, ~scale(log2(. + 1), center = T)) %>% 
  column_to_rownames('marker') %>% as.matrix() %>% t() -> hm_count_data

hm_meta_data <- ucsc.rmsk.salmon_quant$analysis_set$combined$deseq$scaled_quant_meta_for_de.df %>% 
  mutate(type = ifelse(grepl('day7_', sample), 'EB', 'iPSC')) %>% 
  select(sample, condition, type) %>% 
  column_to_rownames('sample')

meta_data <- hm_meta_data[match(colnames(t(hm_count_data)), rownames(hm_meta_data)), , drop = F]


left_hma <- ComplexHeatmap::columnAnnotation(df = meta_data, 
                                          col = list(condition = c('ctrl' = 'white', 'kras' = 'grey'),
                                                     type = c('iPSC' = 'orange', 'EB' = 'blue')), 
                                          show_annotation_name = F, simple_anno_size = unit(.3, "cm"))


lineage_markers %>% 
  remove_rownames() %>% 
  # filter(lineage == 'endoderm') %>% 
  select(-ensg) %>% 
  column_to_rownames('marker') -> marker_data

marker_data <- marker_data[match(colnames(hm_count_data), rownames(marker_data)), , drop = F]

lineage_color_pal <- RColorBrewer::brewer.pal(n = 4, name = 'Set1')
names(lineage_color_pal) <- c('pluripotency', 'mesoderm', 'ectoderm', 'endoderm')

top_hma <- ComplexHeatmap::rowAnnotation(df = marker_data, 
                                            col = list(lineage = lineage_color_pal), 
...
set.seed(129)
ComplexHeatmap::Heatmap(t(hm_count_data), 
                        show_row_names = T, 
                        show_column_names = T, 
                        heatmap_width = figure_double_column_width * 0.85,
                        left_annotation = top_hma, 
...
de_upset.df %>% 
  ...
  xlab('') + ylab('shared DE MYC Target V1 genes') -> fig.S2B.plt
...
de_upset.df %>% 
  ...
  xlab('covid MYC Targets V1 log2FoldChange') + ylab('panc MYC Targets V1 log2FoldChange') +
...
ctrl.6.1.5_nanopore_hex_to.plt %>%
  ...
  # ggforce::facet_col(~sample) +
  ggtitle('nanopore exp. vs obs. size') + 
  scale_fill_viridis_c(option = 'viridis', direction = 1, name = 'scaled cpm', breaks = c(0,1)) +
...
pca.out %>% 
  ...
  geom_hline(yintercept = 0, linetype = 'dotted', alpha = 0.3, size = 0.25) + 
  scale_color_manual(values = identity_color_pal) + 
  annotate(geom = 'text', x = -65, y = -160, label = 'size-aware', size = txt.mm_to_pts(1)) +
